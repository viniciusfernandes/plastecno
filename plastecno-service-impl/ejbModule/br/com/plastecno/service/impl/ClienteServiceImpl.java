...
public class ClienteServiceImpl implements ClienteService {
	...
	public Cliente alterarRevendedor(Cliente cliente) throws BusinessException {
		...
		if (revendedor != null) {
			clienteDAO.alterarTipoCliente(revendedor.getId(),
					TipoCliente.NORMAL);
		}
	...
	public Cliente inserir(Cliente cliente) throws BusinessException {
		...
		if (isNomeFantasiaExistente(cliente.getId(), cliente.getNomeFantasia())) {
			throw new BusinessException(
					"O nome fantasia enviado ja foi cadastrado para outro cliente");
		}

		validarDocumentosPreenchidos(cliente);
		validarRevendedorExistente(cliente);
		inserirEndereco(cliente);
		return cliente.getId() == null ? clienteDAO.inserir(cliente)
				: clienteDAO.alterar(cliente);
	}

	@Override
	public void inserirComentario(Integer idProprietario, Integer idCliente,
	...
	public void inserirComentario(Integer idCliente, String comentario)
		...
		Usuario vendedor = usuarioService
				.pesquisarVendedorByIdCliente(idCliente);
		inserirComentario(vendedor, cliente, comentario);
	}

	private void inserirComentario(Usuario proprietario, Cliente cliente,
			String comentario) throws BusinessException {
	...
	private void inserirEndereco(Cliente cliente) throws BusinessException {
		if (cliente.isListaLogradouroPreenchida()) {
			for (LogradouroCliente logradouro : cliente.getListaLogradouro()) {
				logradouro.addEndereco(this.enderecamentoService
						.inserir(logradouro.recuperarEndereco()));
			}
		}
	}
	...
	public boolean isCNPJExistente(Integer idCliente, String cnpj) {
		return clienteDAO.isEntidadeExistente(Cliente.class, idCliente, "cnpj",
				cnpj);
	}
	...
	public boolean isCPFExistente(Integer idCliente, String cpf) {
		return clienteDAO.isEntidadeExistente(Cliente.class, idCliente, "cpf",
				cpf);
	}
	...
	@Override
	@TransactionAttribute(TransactionAttributeType.SUPPORTS)
	public boolean isInscricaoEstadualExistente(Integer idCliente,
			String inscricaoEstadual) {
		return clienteDAO.isEntidadeExistente(Cliente.class, idCliente,
	...
	public boolean isNomeFantasiaExistente(Integer id, String nomeFantasia) {
		return clienteDAO.isEntidadeExistente(Cliente.class, id,
				"nomeFantasia", nomeFantasia);
	}

	@Override
	@TransactionAttribute(TransactionAttributeType.SUPPORTS)
	public PaginacaoWrapper<Cliente> paginarCliente(Cliente filtro,
			boolean carregarVendedor, Integer indiceRegistroInicial,
			Integer numeroMaximoRegistros) {
		return new PaginacaoWrapper<Cliente>(
				this.pesquisarTotalRegistros(filtro), this.pesquisarBy(filtro,
						true, indiceRegistroInicial, numeroMaximoRegistros));
	}

	@SuppressWarnings("unchecked")
	@Override
	@TransactionAttribute(TransactionAttributeType.SUPPORTS)
	public List<Cliente> pesquisarBy(Cliente filtro, boolean carregarVendedor,
		...
		if (carregarVendedor) {
			select = new StringBuilder(
					"SELECT u FROM Cliente u left join fetch u.vendedor ");
		} else {
		...
		if (indiceRegistroInicial != null && indiceRegistroInicial >= 0
			...
			query.setFirstResult(indiceRegistroInicial).setMaxResults(
	...
	@Override
	@TransactionAttribute(TransactionAttributeType.SUPPORTS)
	public List<Cliente> pesquisarBy(Cliente filtro,
			Integer indiceRegistroInicial, Integer numeroMaximoRegistros) {
		return this.pesquisarBy(filtro, false, indiceRegistroInicial,
				numeroMaximoRegistros);
	}
	...
	public List<Cliente> pesquisarByIdVendedor(Integer idVendedor) {
		StringBuilder select = new StringBuilder(
				"select c from Cliente c left join fetch c.listaContato ");
		if (idVendedor != null) {
	...
	@TransactionAttribute(TransactionAttributeType.SUPPORTS)
	public List<Cliente> pesquisarByIdVendedor(Integer idVendedor,
			boolean isPesquisaClienteInativo) throws BusinessException {
		return isPesquisaClienteInativo ? this
				.pesquisarInativosByIdVendedor(idVendedor) : this
	...
	public List<Cliente> pesquisarByRamoAtividade(Integer idRamoAtividade) {
		...
		return this.entityManager.createQuery(select.toString())
				.setParameter("idRamoAtividade", idRamoAtividade)
				.getResultList();
	}
	...
	@SuppressWarnings("unchecked")
	@TransactionAttribute(TransactionAttributeType.SUPPORTS)
	public List<Cliente> pesquisarClienteByIdRegiao(Integer idRegiao)
			throws BusinessException {

		if (idRegiao == null) {
			throw new BusinessException(
					"Escolha uma região para gerar o relatório de clientes.");
		}

		final List<Integer> listaIdBairro = this.entityManager
				.createQuery(
						"select b.id from Regiao r inner join r.listaBairro b where r.id = :idRegiao")
		...
		return this.entityManager.createQuery(select.toString())
				.setParameter("listaIdBairro", listaIdBairro)
				.setParameter("tipoLogradouro", TipoLogradouro.FATURAMENTO)
				.getResultList();
	}
	...
	public List<Cliente> pesquisarClientesById(List<Integer> listaIdCliente) {
		return this.entityManager
				.createQuery(
						"select c from Cliente c where c.id in (:listaIdCliente)")
	...
	public List<ContatoCliente> pesquisarContato(Integer idCliente) {
		return this.entityManager
				.createQuery(
						"select l from Cliente c inner join c.listaContato l where c.id = :idCliente ")
				.setParameter("idCliente", idCliente).getResultList();
	}
	...
	@Override
	@TransactionAttribute(TransactionAttributeType.SUPPORTS)
	public List<Cliente> pesquisarInativosByIdVendedor(Integer idVendedor)
			throws BusinessException {
		...
		query.setParameter("dataInicioInatividade",
				dataInicioInatividade.getTime());
		return query.getResultList();
	}
	...
	public LogradouroCliente pesquisarLogradouroById(Integer idLogradouro) {
		return this.logradouroService.pesquisarById(idLogradouro,
				LogradouroCliente.class);
	}
	...
	public LogradouroCliente pesquisarLogradouroFaturamentoById(
			Integer idCliente) {
		return clienteDAO.pesquisarLogradouroFaturamentoById(idCliente);
	}
	...
	public Long pesquisarTotalRegistros(Cliente filtro) {
		...
		final StringBuilder select = new StringBuilder(
				"SELECT count(u.id) FROM Cliente u ");
		this.gerarRestricaoPesquisa(filtro, select);
		Query query = this.gerarQueryPesquisa(filtro, select);
	...
	@TransactionAttribute(TransactionAttributeType.SUPPORTS)
	@SuppressWarnings("unchecked")
	public List<Transportadora> pesquisarTransportadorasDesassociadas(
			Integer idCliente) {
		List<Transportadora> listaTransportadora = this
				.pesquisarTransportadorasRedespacho(idCliente);
		Query query = null;
	...
	@SuppressWarnings("unchecked")
	@TransactionAttribute(TransactionAttributeType.SUPPORTS)
	public List<Transportadora> pesquisarTransportadorasRedespacho(
			Integer idCliente) {
		return this.entityManager
				.createQuery(
	...
	public void removerLogradouro(Integer idLogradouro) {
		final LogradouroCliente logradouroCliente = this
				.pesquisarLogradouroById(idLogradouro);
		logradouroCliente.setCancelado(true);
		this.entityManager.merge(logradouroCliente);
	}

	@Override
	public void removerLogradourosAusentes(Integer idCliente,
			Collection<LogradouroCliente> listaLogradouro) {
		final List<LogradouroCliente> listaLogradouroCliente = logradouroService
				.pesquisarAusentes(idCliente, listaLogradouro,
						LogradouroCliente.class);
	...
	private void validarDocumentosPreenchidos(Cliente cliente)
		...
		if (this.isEmailExistente(cliente.getId(), cliente.getEmail())) {
			throw new InformacaoInvalidaException(
					"Email enviado ja foi cadastrado para outro cliente");
		}

		if (this.isCPFExistente(cliente.getId(), cliente.getCpf())) {
			throw new InformacaoInvalidaException(
					"CPF enviado ja foi cadastrado para outro cliente");
		}

		if (this.isCNPJExistente(cliente.getId(), cliente.getCnpj())) {
			throw new InformacaoInvalidaException(
					"CNPJ enviado ja foi cadastrado para outro cliente");
		}

		if (this.isInscricaoEstadualExistente(cliente.getId(),
				cliente.getInscricaoEstadual())) {
			throw new InformacaoInvalidaException(
					"Inscrição Estadual enviado já foi cadastrado para outro cliente");
		}
	}

	@Override
	@TransactionAttribute(TransactionAttributeType.SUPPORTS)
	public void validarListaLogradouroPreenchida(Cliente cliente)
			throws BusinessException {
		logradouroService.validarListaLogradouroPreenchida(cliente
				.getListaLogradouro());
	}

	private void validarRevendedorExistente(Cliente cliente)
			throws BusinessException {
		if (cliente.isRevendedor()
				&& clienteDAO.isRevendedorExistente(cliente.getId())) {
			Cliente revendedor = clienteDAO.pesquisarRevendedor();
			throw new BusinessException(
					"Não é possível mais de um cliente revendedor cadastrado. O cliente \""
							+ revendedor.getNomeFantasia()
							+ "\" já esta cadastrado como revendedor.");
		}
	}
}
