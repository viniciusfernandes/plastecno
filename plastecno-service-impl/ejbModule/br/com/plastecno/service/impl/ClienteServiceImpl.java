...
public class ClienteServiceImpl implements ClienteService {
	...
	public List<Cliente> pesquisarClientesAssociados(Integer idVendedor) {
		return this.entityManager
				.createQuery(
						"select c from Cliente c where c.vendedor.id = :idVendedor")
	...
	public boolean isNomeFantasiaExistente(Integer id, String nomeFantasia) {
		return this.genericDAO.isEntidadeExistente(Cliente.class, id,
	...
	public boolean isClienteProspectado(Integer idCliente) {
		Query query = this.entityManager
				.createQuery(
						"select c.prospeccaoFinalizada from Cliente c where c.id = :idCliente")
	...
	public List<Cliente> pesquisarClientesDesassociados() {
		return this.entityManager.createQuery(
				"select c from Cliente c where c.vendedor = null")
				.getResultList();
	...
	public Cliente inserir(Cliente cliente) throws BusinessException {
		...
		if (cliente.getProspeccaoFinalizada() == null) {
			cliente.setProspeccaoFinalizada(this.isClienteProspectado(cliente
					.getId()));
		}
	...
	public boolean isCNPJExistente(Integer idCliente, String cnpj) {
		return this.genericDAO.isEntidadeExistente(Cliente.class, idCliente,
				"cnpj", cnpj);
	}
	...
	@Override
	@SuppressWarnings("unchecked")
	public List<Transportadora> pesquisarTransportadorasAssociadas(
			Integer idCliente) {
		return this.entityManager
				.createQuery(
						"select t from Cliente c inner join c.listaRedespacho t where c.id = :idCliente and t.ativo = true ")
				.setParameter("idCliente", idCliente).getResultList();
	}
	...
	public List<Transportadora> pesquisarTransportadorasDesassociadas(
		...
		List<Transportadora> listaTransportadora = this
				.pesquisarTransportadorasAssociadas(idCliente);
		Query query = null;
		if (!listaTransportadora.isEmpty()) {
			query = this.entityManager
					.createQuery("select t from Transportadora t where t not in (:listaTransportadora) and t.ativo = true ");
			query.setParameter("listaTransportadora", listaTransportadora);
		} else {
			query = this.entityManager
					.createQuery("select t from Transportadora t ");
		}
		return query.getResultList();
	}
	...
	public boolean isEmailExistente(Integer idCliente, String email) {
		...
		if (idCliente == null) {
			query = this.entityManager
					.createQuery("select count(r.id) from Cliente r where r.email = :email ");
	...
	public List<ComentarioCliente> pesquisarComentarioByIdCliente(
		...
		return (List<ComentarioCliente>) entityManager
				.createQuery(
						"select c from ComentarioCliente c where c.cliente.id = :idCliente ")
				.setParameter("idCliente", idCliente).getResultList();
	}
	...
	private void validarListaLogradouroPreenchida(Cliente cliente)
			throws BusinessException {
		// logradouroService.validarLogradouro(cliente.getListaLogradouro());

		if (cliente.isProspectado()) {
			logradouroService.verificarListaLogradouroObrigatorio(cliente
					.getListaLogradouro());
		}
	}
