...
public class PedidoServiceImpl implements PedidoService {
	...
	public void alterarItemAguardandoCompraByIdPedido(Integer idPedido) {
		pedidoDAO.alterarSituacaoPedidoById(idPedido,
	...
	public void alterarItemAguardandoMaterialByIdPedido(Integer idPedido) {
		pedidoDAO.alterarSituacaoPedidoById(idPedido,
				SituacaoPedido.ITEM_AGUARDANDO_MATERIAL);
	}
	...
	public void alterarQuantidadeRecepcionada(Integer idItemPedido,
		...
		if (!SituacaoPedido.COMPRA_AGUARDANDO_RECEBIMENTO
				.equals(situacaoPedido)) {
			throw new BusinessException(
					"Não é possível alterar a quantidade recepcionada pois a situacao do pedido é \""
							+ situacaoPedido.getDescricao() + "\"");
		}

		Integer quantidadeItem = itemPedidoDAO
				.pesquisarQuantidadeItemPedido(idItemPedido);
		if (quantidadeItem == null) {
			throw new BusinessException("O item de pedido de código "
					+ idItemPedido + " pesquisado não existe no sistema");
		}

		if (quantidadeItem < quantidadeRecepcionada) {
			Integer idPedido = pesquisarIdPedidoByIdItemPedido(idItemPedido);
			Integer sequencialItem = itemPedidoDAO
					.pesquisarSequencialItemPedido(idItemPedido);
			throw new BusinessException(
					"Não é possível recepcionar uma quantidade maior do que foi comprado para o item No. "
							+ sequencialItem + " do pedido No. " + idPedido);
		}
		itemPedidoDAO.alterarQuantidadeRecepcionada(idItemPedido,
	...
	public void alterarQuantidadeReservadaByIdItemPedido(Integer idItemPedido) {
		entityManager
				.createQuery(
						"update ItemPedido i set i.quantidadeReservada = 0 where i.id=:id")
	...
	public void alterarRevendaAguardandoMaterialByIdItem(Integer idItemPedido) {
		alterarSituacaoPedidoByIdItemPedido(idItemPedido,
				SituacaoPedido.ITEM_AGUARDANDO_MATERIAL);
	}
	...
	@Override
	@TransactionAttribute(TransactionAttributeType.REQUIRED)
	public void alterarSituacaoPedidoByIdItemPedido(Integer idItemPedido,
	...
	@Override
	@TransactionAttribute(TransactionAttributeType.REQUIRED)
	public void alterarSituacaoPedidoByIdPedido(Integer idPedido,
			SituacaoPedido situacaoPedido) {
		pedidoDAO.alterarSituacaoPedidoById(idPedido, situacaoPedido);
	}
	...
	private void calcularComissaoVenda(Pedido pedido) throws BusinessException {
		...
		if (pedido.isRepresentacao() && pedido.getAliquotaComissao() == null) {
			throw new BusinessException(
					"Não existe comissão configurada para o pedido pedido No. "
							+ pedido.getId()
							+ ". Veja as configurações da representada \""
							+ pedido.getRepresentada().getNomeFantasia() + "\"");
		}

		List<ItemPedido> listaItem = pesquisarItemPedidoByIdPedido(pedido
				.getId());
		...
		for (ItemPedido itemPedido : listaItem) {
			...
			if (itemPedido.contemAliquotaComissao()) {
				...
				comissaoVenda.setAliquotaRevenda(itemPedido
						.getAliquotaComissao());
				comissaoVenda.setAliquotaRepresentacao(itemPedido
						.getAliquotaComissao());
			} else if (pedido.isRevenda()
					&& !itemPedido.contemAliquotaComissao()) {
				comissaoVenda = comissaoService
						.pesquisarComissaoVigenteProduto(itemPedido
								.getMaterial().getId(), itemPedido
				...
				if (comissaoVenda == null) {
					comissaoVenda = comissaoService
							.pesquisarComissaoVigenteVendedor(pedido
									.getVendedor().getId());
				}

			} else if (pedido.isRepresentacao()) {
				comissaoVenda = comissaoService
						.pesquisarComissaoVigenteVendedor(pedido.getVendedor()
								.getId());
			}
			...
			if (pedido.isRevenda() && comissaoVenda != null
					&& comissaoVenda.getAliquotaRevenda() != null) {
				valorComissionado = precoItem
						* comissaoVenda.getAliquotaRevenda();
			} else if (pedido.isRepresentacao() && comissaoVenda != null
					&& comissaoVenda.getAliquotaRepresentacao() != null) {
				valorComissionado = precoItem
						* comissaoVenda.getAliquotaRepresentacao();
				valorComissionadoRepresentacao = precoItem
						* (pedido.getRepresentada().getComissao());
			} else {
				Usuario vendedor = usuarioService
						.pesquisarUsuarioResumidoById(pedido.getVendedor()
								.getId());
				throw new BusinessException(
	...
	@Override
	@TransactionAttribute(TransactionAttributeType.SUPPORTS)
	public Double calcularValorPedido(Integer idPedido)
			throws BusinessException {
		try {
			return pedidoDAO.pesquisarQuantidadePrecoUnidade(idPedido);
		} catch (PersistenceException e) {
			throw new BusinessException(
					"Falha no calculo do valor da unidade do item do pedido "
							+ idPedido
	...
	@Override
	@TransactionAttribute(TransactionAttributeType.SUPPORTS)
	public Double calcularValorPedidoIPI(Integer idPedido)
			throws BusinessException {
		try {
			return pedidoDAO.pesquisarQuantidadePrecoUnidadeIPI(idPedido);
		} catch (PersistenceException e) {
			throw new BusinessException(
					"Falha no calculo do valor IPI do item do pedido "
							+ idPedido
							+ ". Provavelmente o valor do item esta estourando os limites do sistema.");
		}
	}
	...
	public void cancelarPedido(Integer idPedido) throws BusinessException {
		if (idPedido == null) {
			throw new BusinessException(
					"Não é possível cancelar o pedido pois ele não existe no sistema");
		}
	...
	@Override
	@TransactionAttribute(TransactionAttributeType.REQUIRED)
	public Integer comprarItemPedido(Integer idComprador,
			Integer idRepresentadaFornecedora, Set<Integer> listaIdItemPedido)
			throws BusinessException {
		if (listaIdItemPedido == null || listaIdItemPedido.isEmpty()) {
			throw new BusinessException(
					"A lista de itens de pedido para encomendar não pode estar vazia");
		}
		Representada fornecedor = representadaService
				.pesquisarById(idRepresentadaFornecedora);
		if (fornecedor == null) {
			throw new BusinessException(
					"Fornecedor é obrigatório para realizar a encomenda");
		}

		if (!fornecedor.isFornecedor()) {
			throw new BusinessException("A fornecedor \""
					+ fornecedor.getNomeFantasia()
					+ "\" escolhido não esta cadastrado como fornecedor");
		}

		verificarMaterialAssociadoFornecedor(idRepresentadaFornecedora,
				listaIdItemPedido);
		...
		for (Integer idItemPedido : listaIdItemPedido) {
			...
			itemClone.setIdPedidoVenda(pedidoDAO
					.pesquisarIdPedidoByIdItemPedido(idItemPedido));
			...
			try {
				...
				throw new BusinessException(
						"Não foi possível cadastrar uma nova encomenda pois houve falha no item No. "
								+ itemCadastrado.getSequencial()
								+ " do pedido No. "
								+ itemCadastrado.getPedido().getId()
								+ ". Possível problema: "
			...
			if (!contemPedidoItemRevendaAguardandoEncomenda(idItemPedido)) {
				...
				alterarItemAguardandoMaterialByIdPedido(itemCadastrado
						.getPedido().getId());
			}
	...
	public boolean contemPedidoItemRevendaAguardandoEncomenda(
	...
	@Override
	@TransactionAttribute(TransactionAttributeType.SUPPORTS)
	public boolean contemQuantidadeNaoRecepcionadaItemPedido(
			Integer idItemPedido) {
	...
	private void definirTipoPedido(Pedido pedido) {
		...
		if (pedido.getTipoPedido() == null) {
			if (representadaService.isRevendedor(pedido.getRepresentada()
					.getId())) {
	...
	public boolean empacotarItemAguardandoCompra(Integer idPedido)
			throws BusinessException {
	...
	@Override
	@TransactionAttribute(TransactionAttributeType.REQUIRES_NEW)
	public boolean empacotarItemAguardandoMaterial(Integer idPedido)
			throws BusinessException {
		boolean empacotamentoOk = estoqueService.reservarItemPedido(idPedido);
		if (!empacotamentoOk) {
	...
	@Override
	public boolean empacotarPedidoAguardandoCompra(Integer idPedido)
			throws BusinessException {
		boolean empacotamentoOk = estoqueService.reservarItemPedido(idPedido);
		if (!empacotamentoOk) {
			alterarItemAguardandoCompraByIdPedido(idPedido);
	...
	private void enviarOrcamento(Pedido pedido, byte[] arquivoAnexado)
			throws BusinessException {
		Contato contato = pedido.getContato();
		if (StringUtils.isEmpty(contato.getEmail())) {
			throw new BusinessException(
					"Email do contato é obrigatório para envio do orçamento");
		}

		if (StringUtils.isEmpty(contato.getDdd())) {
			throw new BusinessException(
					"DDD do telefone do contato é obrigatório para envio do orçamento");
		}

		if (StringUtils.isEmpty(contato.getTelefone())) {
			throw new BusinessException(
					"O telefone do contato é obrigatório para envio do orçamento");
		}
		...
		try {
			...
			mensagem.append("Falha no envio do orçamento No. ")
					.append(pedido.getId())
					.append(" do vendedor ")
					.append(pedido.getVendedor().getNomeCompleto())
					.append(" para o cliente ")
					.append(pedido.getCliente().getNomeCompleto())
					.append(" e contato feito por "
							+ pedido.getContato().getNome());
	...
	@Override
	@TransactionAttribute(TransactionAttributeType.REQUIRED)
	public void enviarPedido(Integer idPedido, byte[] arquivoAnexado)
			throws BusinessException {
		 ...
		 */
		pedido.addLogradouro(clienteService.pesquisarLogradouro(pedido
				.getCliente().getId()));
	...
	private void enviarVenda(Pedido pedido, byte[] arquivoAnexado)
			throws BusinessException {
		...
		logradouroService.validarListaLogradouroPreenchida(pedido
				.getListaLogradouro());
		try {
			GeradorPedidoEmail gerador = new GeradorPedidoEmail(pedido,
					arquivoAnexado);
			emailService
					.enviar(gerador.gerarMensagem(TipoMensagemPedido.VENDA));

			if (pedido.isClienteNotificadoVenda()) {
				emailService.enviar(gerador
						.gerarMensagem(TipoMensagemPedido.VENDA_CLIENTE));
			}

		} catch (NotificacaoException e) {
			StringBuilder mensagem = new StringBuilder();
			mensagem.append("Falha no envio do pedido No. ")
					.append(pedido.getId()).append(" do vendedor ")
					.append(pedido.getVendedor().getNomeCompleto())
					.append(" para a representada ")
	...
	public Pedido inserir(Pedido pedido) throws BusinessException {
		if (SituacaoPedido.CANCELADO.equals(pedido.getSituacaoPedido())) {
			throw new InformacaoInvalidaException(
					"Pedido ja foi cancelado e nao pode ser alterado");
		}
		...
		if (isPedidoNovo
				&& pedido.isVenda()
				&& !this.usuarioService.isVendaPermitida(pedido.getCliente()
						.getId(), pedido.getVendedor().getId())) {

			Cliente cliente = this.clienteService.pesquisarById(pedido
					.getCliente().getId());
			Usuario proprietario = this.usuarioService.pesquisarById(pedido
					.getProprietario().getId());
			throw new BusinessException(
					"Não é possível incluir o pedido pois o cliente "
							+ (cliente != null ? cliente.getNomeCompleto()
									: pedido.getCliente().getId())
							+ " não esta associado ao vendedor "
							+ (proprietario != null ? proprietario.getNome()
									+ " - " + proprietario.getEmail() : pedido
									.getCliente().getId()));
		}
		...
		if (pedido.isVenda()) {
			...
			Usuario vendedor = usuarioService
					.pesquisarVendedorByIdCliente(pedido.getCliente().getId());
			if (vendedor == null) {
				String nomeCliente = clienteService
						.pesquisarNomeFantasia(pedido.getCliente().getId());
				throw new BusinessException(
						"Não existe vendedor associado ao cliente "
								+ nomeCliente);
			}
			pedido.setVendedor(vendedor);
			pedido.setAliquotaComissao(representadaService
					.pesquisarComissaoRepresentada(pedido.getRepresentada()
							.getId()));
		}

		final Date dataEntrega = DateUtils.gerarDataSemHorario(pedido
				.getDataEntrega());
		if (SituacaoPedido.DIGITACAO.equals(pedido.getSituacaoPedido())
				&& dataEntrega != null
				&& DateUtils.isAnteriorDataAtual(dataEntrega)) {
			throw new InformacaoInvalidaException(
					"Data de entrega deve ser posterior a data atual");
		}

		if (!pedido.isOrcamento()
				&& TipoEntrega.CIF_TRANS.equals(pedido.getTipoEntrega())
				&& pedido.getTransportadoraRedespacho() == null) {
			throw new BusinessException(
					"A transportadora de redespacho é obrigatória para o tipo de entrega "
		 ...
		 * logradouro
		 */
		pedido.addLogradouro(this.clienteService.pesquisarLogradouro(pedido
				.getCliente().getId()));
	...
	@Override
	@TransactionAttribute(TransactionAttributeType.REQUIRED)
	public Integer inserirItemPedido(Integer idPedido, ItemPedido itemPedido)
			throws BusinessException {
		// configurando o material para efetuar o calculo usando o peso
		// especifico
		if (itemPedido.getMaterial() != null) {
			itemPedido.setMaterial(this.materialService
					.pesquisarById(itemPedido.getMaterial().getId()));
		}

		if (itemPedido.isPeca() && itemPedido.isVendaKilo()) {
			throw new BusinessException(
					"Não é possível vender uma peca por kilo");
		}

		if (itemPedido.isPeca()
				&& StringUtils.isEmpty(itemPedido.getDescricaoPeca())) {
			throw new BusinessException(
					"Descrição da peca do item do pedido é obrigatório");
		}
		 ...
		 */
		itemPedido.setPrecoUnidade(CalculadoraPreco
				.calcularPorUnidade(itemPedido));
		...
		final boolean ipiObrigatorio = TipoApresentacaoIPI.SEMPRE
				.equals(tipoApresentacaoIPI);
		final boolean ipiImportado = TipoApresentacaoIPI.OCASIONAL
				.equals(tipoApresentacaoIPI)
				&& materialService.isMaterialImportado(itemPedido.getMaterial()
						.getId());
		...
		final Double precoUnidadeIPI = CalculadoraPreco
				.calcularPorUnidadeIPI(itemPedido);

		itemPedido.setPrecoUnidadeIPI(precoUnidadeIPI);
		itemPedido.setPrecoMinimo(NumeroUtils
				.arredondarValorMonetario(estoqueService
						.calcularPrecoMinimoItemEstoque(itemPedido)));
		itemPedido.setPrecoCusto(estoqueService
				.calcularPrecoCustoItemEstoque(itemPedido));
		...
		if (itemPedido.isNovo()) {
			...
			if (pedido.isCompra()) {
				...
				Object[] idPedidoCompraEVenda = itemPedidoDAO
						.pesquisarIdPedidoCompraEVenda(itemPedido.getId());
	...
	public Integer inserirItemPedido(ItemPedido itemPedido)
			throws BusinessException {
		Integer idPedido = pedidoDAO.pesquisarIdPedidoByIdItemPedido(itemPedido
				.getId());
		if (idPedido == null) {
			throw new BusinessException(
					"Não existe pedido cadastrado para o item "
							+ (itemPedido.isPeca() ? itemPedido
									.getDescricaoPeca() : itemPedido
									.getDescricao()));
		}
	...
	@Override
	@TransactionAttribute(TransactionAttributeType.REQUIRED)
	public void inserirNcmItemAguardandoMaterialAssociadoByIdItemCompra(
			Integer idItemPedidoCompra, String ncm) throws BusinessException {
		if (idItemPedidoCompra == null || ncm == null || ncm.isEmpty()) {
			return;
		}
		try {
			itemPedidoDAO.inserirNcmItemAguardandoMaterialAssociadoItemCompra(
					idItemPedidoCompra, ncm);
		} catch (Exception e) {
			throw new BusinessException(
					"Falha na inclusao do NCM nos item de revenda associados ao pedido No. "
							+ pesquisarIdPedidoByIdItemPedido(idItemPedidoCompra),
					e);
		}
	}
	...
	public Pedido inserirOrcamento(Pedido pedido) throws BusinessException {
		...
		cliente.setRamoAtividade(ramoAtividadeService
				.pesquisarRamoAtividadePadrao());
	...
	public boolean isPedidoEnviado(Integer idPedido) {
		SituacaoPedido situacao = QueryUtil
				.gerarRegistroUnico(
						this.entityManager
								.createQuery(
										"select p.situacaoPedido from Pedido p where p.id = :idPedido")
	...
	@Override
	public PaginacaoWrapper<Pedido> paginarPedido(Integer idCliente,
			Integer idVendedor, Integer idFornecedor, boolean isCompra,
			Integer indiceRegistroInicial, Integer numeroMaximoRegistros) {
		List<Pedido> listaPedido = null;
		if (idVendedor == null
				|| usuarioService.isVendaPermitida(idCliente, idVendedor)) {
			listaPedido = pesquisarByIdClienteIdFornecedor(idCliente,
					idFornecedor, isCompra, indiceRegistroInicial,
		...
		return new PaginacaoWrapper<Pedido>(
				pesquisarTotalPedidoByIdClienteIdVendedorIdFornecedor(
						idCliente, idVendedor, idFornecedor, isCompra, null),
				listaPedido);
	}
	...
	@Override
	@TransactionAttribute(TransactionAttributeType.SUPPORTS)
	public List<Pedido> pesquisarBy(Pedido filtro,
			Integer indiceRegistroInicial, Integer numeroMaximoRegistros) {
		if (filtro == null) {
			return Collections.emptyList();
		}
		return pedidoDAO.pesquisarBy(filtro, indiceRegistroInicial,
				numeroMaximoRegistros);
	}
	...
	public List<Pedido> pesquisarByIdCliente(Integer idCliente,
			Integer indiceRegistroInicial, Integer numeroMaximoRegistros) {
		return pesquisarByIdClienteIdFornecedor(idCliente, null, false,
				indiceRegistroInicial, numeroMaximoRegistros);
	}

	@Override
	@TransactionAttribute(TransactionAttributeType.SUPPORTS)
	public List<Pedido> pesquisarByIdClienteIdFornecedor(Integer idCliente,
			Integer idFornecedor, boolean isCompra,
			Integer indiceRegistroInicial, Integer numeroMaximoRegistros) {
		return this.pesquisarPedidoByIdClienteIdVendedorIdFornecedor(idCliente,
				null, idFornecedor, isCompra, indiceRegistroInicial,
				numeroMaximoRegistros);
	}
	...
	@Override
	@TransactionAttribute(TransactionAttributeType.SUPPORTS)
	public List<ItemPedido> pesquisarCompraAguardandoRecebimento(
			Integer idRepresentada, Periodo periodo) {
		if (periodo != null) {
			return itemPedidoDAO.pesquisarCompraAguardandoRecebimento(
					idRepresentada, periodo.getInicio(), periodo.getFim());
		}
		return itemPedidoDAO.pesquisarCompraAguardandoRecebimento(
				idRepresentada, null, null);
	}
	...
	@Override
	@TransactionAttribute(TransactionAttributeType.SUPPORTS)
	public List<Pedido> pesquisarCompraByPeriodoEComprador(Periodo periodo,
			Integer idComprador) throws BusinessException {
		return pesquisarPedidoEnviadoByPeriodoEProprietario(false, periodo,
				idComprador, true);
	}
	...
	@SuppressWarnings("unchecked")
	@TransactionAttribute(TransactionAttributeType.SUPPORTS)
	public List<Pedido> pesquisarEnviadosByPeriodoERepresentada(
			Periodo periodo, Integer idRepresentada) {
		StringBuilder select = new StringBuilder()
				...
				.append(" p.dataEnvio >= :dataInicio and ")
				.append(" p.dataEnvio <= :dataFim and ")
	...
	@Override
	@TransactionAttribute(TransactionAttributeType.SUPPORTS)
	public List<Pedido> pesquisarEnviadosByPeriodoEVendedor(Periodo periodo,
	...
	@Override
	@TransactionAttribute(TransactionAttributeType.SUPPORTS)
	public Object[] pesquisarIdMaterialFormaMaterialItemPedido(
			Integer idItemPedido) {
		return itemPedidoDAO
				.pesquisarIdMaterialFormaMaterialItemPedido(idItemPedido);
	}
	...
	public List<Integer> pesquisarIdPedidoAguardandoCompra() {
		return pedidoDAO
				.pesquisarIdPedidoBySituacaoPedido(SituacaoPedido.ITEM_AGUARDANDO_COMPRA);
	}
	...
	public List<Integer> pesquisarIdPedidoAguardandoEmpacotamento() {
		return pedidoDAO
				.pesquisarIdPedidoBySituacaoPedido(SituacaoPedido.REVENDA_AGUARDANDO_EMPACOTAMENTO);
	}
	...
	public List<Integer> pesquisarIdPedidoAguardandoMaterial() {
		return pedidoDAO
				.pesquisarIdPedidoBySituacaoPedido(SituacaoPedido.ITEM_AGUARDANDO_MATERIAL);
	}

	@Override
	@TransactionAttribute(TransactionAttributeType.SUPPORTS)
	public List<Integer> pesquisarIdPedidoAssociadoByIdPedidoOrigem(
			Integer idPedidoOrigem, boolean isCompra) {
		if (idPedidoOrigem == null) {
			return new ArrayList<Integer>();
		}
		return itemPedidoDAO.pesquisarIdPedidoAssociadoByIdPedidoOrigem(
				idPedidoOrigem, isCompra);
	}
	...
	@Override
	@TransactionAttribute(TransactionAttributeType.SUPPORTS)
	public List<Integer> pesquisarIdPedidoByIdItemPedido(
	...
	public Integer pesquisarIdVendedorByIdPedido(Integer idPedido) {
		...
		return QueryUtil
				.gerarRegistroUnico(
						this.entityManager
								.createQuery(
										"select v.id from Pedido p inner join p.proprietario v where p.id = :idPedido ")
								.setParameter("idPedido", idPedido),
						Integer.class, null);
	}
	...
	@Override
	@TransactionAttribute(TransactionAttributeType.SUPPORTS)
	public List<ItemPedido> pesquisarItemAguardandoCompra(Integer idCliente,
			Periodo periodo) {
		if (periodo != null) {
			return itemPedidoDAO.pesquisarItemAguardandoCompra(idCliente,
					periodo.getInicio(), periodo.getFim());
		}
		return itemPedidoDAO.pesquisarItemAguardandoCompra(idCliente, null,
				null);
	}

	@Override
	@TransactionAttribute(TransactionAttributeType.SUPPORTS)
	public List<ItemPedido> pesquisarItemAguardandoMaterial(
			Integer idRepresentada, Periodo periodo) {
		if (periodo != null) {
			return itemPedidoDAO.pesquisarItemAguardandoMaterial(
					idRepresentada, periodo.getInicio(), periodo.getFim());
		}
		return itemPedidoDAO.pesquisarItemAguardandoMaterial(idRepresentada,
				null, null);
	}
	...
	public List<ItemPedido> pesquisarItemPedidoAguardandoEmpacotamento(
			Integer idCliente) {
		return itemPedidoDAO
				.pesquisarItemPedidoAguardandoEmpacotamento(idCliente);
	}
	...
	@Override
	@TransactionAttribute(TransactionAttributeType.SUPPORTS)
	public List<ItemPedido> pesquisarItemPedidoByIdClienteIdVendedorIdFornecedor(
		...
		return itemPedidoDAO
				.pesquisarItemPedidoByIdClienteIdVendedorIdFornecedor(
						idCliente, idVendedor, idFornecedor, isCompra,
						indiceRegistroInicial, numeroMaximoRegistros,
						itemVendido);
	}
	...
	@Override
	@TransactionAttribute(TransactionAttributeType.SUPPORTS)
	public List<ItemPedido> pesquisarItemPedidoCompradoResumidoByPeriodo(
			Periodo periodo) {
		return pesquisarValoresItemPedidoResumidoByPeriodo(periodo,
				pesquisarSituacaoCompraEfetivada(), TipoPedido.COMPRA);
	}
	...
	@Override
	@TransactionAttribute(TransactionAttributeType.SUPPORTS)
	public List<ItemPedido> pesquisarItemPedidoEncomendado(Integer idCliente,
			Date dataInicial, Date dataFinal) {
		return itemPedidoDAO.pesquisarItemPedidoAguardandoMaterial(idCliente,
				dataInicial, dataFinal);
	}

	@Override
	@TransactionAttribute(TransactionAttributeType.SUPPORTS)
	public List<ItemPedido> pesquisarItemPedidoRepresentacaoByPeriodo(
			Periodo periodo) {
		return pesquisarValoresItemPedidoResumidoByPeriodo(periodo,
				pesquisarSituacaoVendaEfetivada(), TipoPedido.REPRESENTACAO);
	}
	...
	public List<ItemPedido> pesquisarItemPedidoRevendaByPeriodo(Periodo periodo) {
		return pesquisarValoresItemPedidoResumidoByPeriodo(periodo,
				pesquisarSituacaoVendaEfetivada(), TipoPedido.REVENDA);
	}

	@Override
	public List<ItemPedido> pesquisarItemPedidoVendaByPeriodo(Periodo periodo,
			Integer idVendedor) {
		if (idVendedor == null) {
			return new ArrayList<ItemPedido>();
		}

		return itemPedidoDAO.pesquisarItemPedidoVendaComissionadaByPeriodo(
				periodo, idVendedor, pesquisarSituacaoVendaEfetivada());
	}

	@Override
	public List<ItemPedido> pesquisarItemPedidoVendaResumidaByPeriodo(
			Periodo periodo) {
		return itemPedidoDAO.pesquisarItemPedidoVendaComissionadaByPeriodo(
				periodo, null, pesquisarSituacaoVendaEfetivada());
	}
	...
	@Override
	@TransactionAttribute(TransactionAttributeType.SUPPORTS)
	public List<Pedido> pesquisarPedidoByIdCliente(Integer idCliente,
			Integer indiceRegistroInicial, Integer numeroMaximoRegistros) {

		if (idCliente == null) {
			return Collections.emptyList();
		}
		return pedidoDAO.pesquisarPedidoByIdCliente(idCliente,
				indiceRegistroInicial, numeroMaximoRegistros);
	}

	@Override
	@TransactionAttribute(TransactionAttributeType.SUPPORTS)
	public List<Pedido> pesquisarPedidoByIdClienteIdVendedorIdFornecedor(
			Integer idCliente, Integer idVendedor, Integer idFornecedor,
			boolean isCompra, Integer indiceRegistroInicial,
		...
		return this.pedidoDAO.pesquisarPedidoByIdClienteIdVendedorIdFornecedor(
				idCliente, idVendedor, idFornecedor, isCompra,
				indiceRegistroInicial, numeroMaximoRegistros);
	}
	...
	@SuppressWarnings("unchecked")
	private List<Pedido> pesquisarPedidoEnviadoByPeriodoEProprietario(
			boolean orcamento, Periodo periodo, Integer idProprietario,
		...
		select.append(
				...
				.append("where p.situacaoPedido IN :situacoes and ")
				.append("p.proprietario.id = :idProprietario and ")
				.append(" p.dataEnvio >= :dataInicio and ")
	...
	public int pesquisarQuantidadeNaoRecepcionadaItemPedido(Integer idItemPedido) {
		return pesquisarQuantidadeItemPedido(idItemPedido)
				- pesquisarQuantidadeRecepcionadaItemPedido(idItemPedido);
	}

	@Override
	@TransactionAttribute(TransactionAttributeType.SUPPORTS)
	public int pesquisarQuantidadeRecepcionadaItemPedido(Integer idItemPedido) {
		Integer q = itemPedidoDAO
				.pesquisarQuantidadeRecepcionadaItemPedido(idItemPedido);
		return q == null ? 0 : q;
	}
	...
	private TipoApresentacaoIPI pesquisarTipoApresentacaoIPI(
			ItemPedido itemPedido) throws BusinessException {
		if (itemPedido.getPedido() == null
				|| itemPedido.getPedido().getId() == null) {
			throw new BusinessException(
		...
		final Integer idRepresentada = pedidoDAO
				.pesquisarIdRepresentadaByIdPedido(itemPedido.getPedido()
						.getId());
	...
	@Override
	@TransactionAttribute(TransactionAttributeType.SUPPORTS)
	public List<TotalizacaoPedidoWrapper> pesquisarTotalCompraResumidaByPeriodo(
			Periodo periodo) {
		return pedidoDAO.pesquisarValorTotalPedidoByPeriodo(
				periodo.getInicio(), periodo.getFim(), true);
	}
	...
	public Long pesquisarTotalItemRevendaAguardandoEncomenda(
			Integer idItemPedido) {
		Integer idPedido = pesquisarIdPedidoByIdItemPedido(idItemPedido);
	...
	@Override
	@TransactionAttribute(TransactionAttributeType.SUPPORTS)
	public Long pesquisarTotalPedidoByIdClienteIdFornecedor(Integer idCliente,
			Integer idFornecedor, boolean isCompra) {
		return pesquisarTotalPedidoByIdClienteIdVendedorIdFornecedor(idCliente,
				null, idFornecedor, isCompra, null);
	}

	@Override
	@TransactionAttribute(TransactionAttributeType.SUPPORTS)
	public Long pesquisarTotalPedidoByIdClienteIdVendedorIdFornecedor(
			Integer idCliente, Integer idVendedor, Integer idFornecedor,
	...
	@Override
	@TransactionAttribute(TransactionAttributeType.SUPPORTS)
	public Long pesquisarTotalPedidoVendaByIdClienteIdVendedorIdFornecedor(
			Integer idCliente) {
		return this.pesquisarTotalPedidoByIdClienteIdVendedorIdFornecedor(
				idCliente, null, null, false, null);
	}

	@Override
	@TransactionAttribute(TransactionAttributeType.SUPPORTS)
	public List<TotalizacaoPedidoWrapper> pesquisarTotalPedidoVendaResumidaByPeriodo(
			Periodo periodo) {
		return pedidoDAO.pesquisarValorTotalPedidoByPeriodo(
	...
	private List<ItemPedido> pesquisarValoresItemPedidoResumidoByPeriodo(
			Periodo periodo, List<SituacaoPedido> listaSituacao,
		...
		return this.entityManager
				.createQuery(select.toString(), ItemPedido.class)
				.setParameter("dataInicio", periodo.getInicio())
				.setParameter("dataFim", periodo.getFim())
				.setParameter("situacoes", listaSituacao)
				.setParameter("tipoPedido", tipoPedido).getResultList();
	}
	...
	@Override
	@TransactionAttribute(TransactionAttributeType.SUPPORTS)
	public List<TotalizacaoPedidoWrapper> pesquisarValorVendaClienteByPeriodo(
		...
		List<Object[]> resultado = pedidoDAO
				.pesquisarValorVendaClienteByPeriodo(periodo.getInicio(),
						periodo.getFim(), idCliente, isOrcamento);
		for (Object[] o : resultado) {
			listaTotalizacao.add(new TotalizacaoPedidoWrapper((String) o[2],
					(Long) o[0], (Double) o[1]));
		}
	...
	@Override
	@TransactionAttribute(TransactionAttributeType.SUPPORTS)
	public List<Pedido> pesquisarVendaByPeriodoEVendedor(boolean orcamento,
			Periodo periodo, Integer idVendedor) throws BusinessException {
		return pesquisarPedidoEnviadoByPeriodoEProprietario(orcamento, periodo,
				idVendedor, false);
	}
	...
	@Override
	@TransactionAttribute(TransactionAttributeType.REQUIRED)
	public void reencomendarItemPedido(Integer idItemPedido)
			throws BusinessException {
		alterarSituacaoPedidoByIdItemPedido(idItemPedido,
				SituacaoPedido.ITEM_AGUARDANDO_COMPRA);
	...
	public Integer refazerPedido(Integer idPedido) throws BusinessException {
		...
		try {
			...
			throw new BusinessException(
					"Falha no processo de copia do pedido No. " + idPedido, e);
		}
		...
		for (ItemPedido itemPedido : listaItemPedido) {
			try {
				...
				throw new BusinessException(
						"Falha no processo de copia do item No. "
								+ itemPedido.getId() + " do pedido No. "
								+ idPedido, e);
			}
		}
	...
	@Override
	public Pedido removerItemPedido(Integer idItemPedido)
			throws BusinessException {
		...
		try {
			...
			alterarSequencialItemPedido(pedido.getId(),
					itemPedido.getSequencial());
			...
			if (pedido.isCompraEfetuada()
					&& pesquisarTotalItemPedido(pedido.getId()) <= 0L) {
			...
			throw new BusinessException(
					"Não foi possivel remover o item pois não existe item com o codigo "
							+ idItemPedido);
		}
	...
	private void validarEnvio(Pedido pedido) throws BusinessException {
		...
		if (!pedido.isOrcamento()) {
			clienteService
					.validarListaLogradouroPreenchida(pedido.getCliente());
		}
		...
		if (SituacaoPedido.CANCELADO.equals(pedido.getSituacaoPedido())
			...
			exception
					.addMensagem("Pedido cancelado ou enviado não pode ser enviado para as representadas");
		}
	...
	private void validarEnvioVenda(Pedido pedido) throws BusinessException {
		...
		if (DATA_ENTREGA != null && DATA_ENTREGA.compareTo(DATA_ATUAL) < 0) {
			exception
					.addMensagem("Data de entrega deve ser posterior a data atual");
		}
	...
	private void verificarMaterialAssociadoFornecedor(
			Integer idRepresentadaFornecedora, Set<Integer> listaIdItemPedido)
			throws BusinessException {
		Integer idMaterial = null;
		for (Integer idItemPedido : listaIdItemPedido) {

			idMaterial = itemPedidoDAO
					.pesquisarIdMeterialByIdItemPedido(idItemPedido);
			if (!materialService.isMaterialAssociadoRepresentada(idMaterial,
					idRepresentadaFornecedora)) {
				Integer idPedido = pesquisarIdPedidoByIdItemPedido(idItemPedido);
				Integer sequencial = itemPedidoDAO
						.pesquisarSequencialItemPedido(idItemPedido);
				String nomeFantasia = representadaService
						.pesquisarNomeFantasiaById(idRepresentadaFornecedora);
				throw new BusinessException(
						"Não é possível encomendar o item No. " + sequencial
								+ " do pedido No. " + idPedido
								+ " pois o fornecedor \"" + nomeFantasia
								+ "\" não trabalha com o material do item");
			}
		}
	}
}
