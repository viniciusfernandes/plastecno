...
public class PedidoServiceImpl implements PedidoService {
	...
	public List<Pedido> pesquisarByPeriodoEVendedor(boolean orcamento, Periodo periodo, Integer idVendedor) throws BusinessException {
		if(idVendedor == null) {
		...
		.append("p.vendedor.id = :idVendedor and ")
		.append(" p.dataEnvio >= :dataInicio and ")
		.append(" p.dataEnvio <= :dataFim ")
		.append("order by p.dataEnvio desc ");
	...
	public List<Pedido> pesquisarEnviadosByPeriodoERepresentada(Periodo periodo, Integer idRepresentada) {
		...
		.append("p.representada.id = :idRepresentada and ")
		.append(" p.dataEnvio >= :dataInicio and ")
		.append(" p.dataEnvio <= :dataFim ")
		.append("order by p.dataEnvio desc ");
		
		return this.entityManager.createQuery(select.toString())
				.setParameter("situacaoPedido", SituacaoPedido.ENVIADO)
	...
	@Override
	public PaginacaoWrapper<Pedido> paginarPedido(Integer idCliente, Integer indiceRegistroInicial,  Integer numeroMaximoRegistros) {
		return paginarPedido(idCliente, null, indiceRegistroInicial, numeroMaximoRegistros);
	}
	
	@Override
	public PaginacaoWrapper<Pedido> paginarPedido(Integer idCliente, Integer idVendedor, Integer indiceRegistroInicial,  Integer numeroMaximoRegistros) {
	...
	public void cancelar(Integer idPedido) throws BusinessException{
		...
		this.entityManager.createQuery("update Pedido p set p.situacaoPedido = :situacao where p.id = :idPedido")
			.setParameter("situacao", SituacaoPedido.CANCELADO)
	...
	/*
	 * Esse metodo retorna um pedido pois, apos a inclusao de um novo pedido, configuramos 
	 * a data de inclusao como sendo a data atual, e essa informacao deve ser retornada para
	 * o componente chamador.
	 */
	@Override
	public Pedido inserir(Pedido pedido) throws BusinessException {
		...
		if (isPedidoNovo && 
			...
			throw new BusinessException("Não é possível incluir o pedido pois o cliente "+ (cliente != null ? cliente.getNomeCompleto() : pedido.getCliente().getId())
					+" não esta associado ao vendedor "+(vendedor != null ? vendedor.getNome() +" - "+vendedor.getEmail(): pedido.getCliente().getId()));
		}
		
		final Date dataEntrega = DateUtils.gerarDataSemHorario(pedido.getDataEntrega()) ;
		if (!SituacaoPedido.CANCELADO.equals(pedido.getSituacaoPedido()) && dataEntrega != null && DateUtils.isAnteriorDataAtual(dataEntrega)) {
		...
		if (TipoEntrega.CIF_TRANS.equals(pedido.getTipoEntrega()) && pedido.getTransportadoraRedespacho() == null) {
			throw new BusinessException("A transportadora de redespacho é obrigatória para o tipo de entrega "+TipoEntrega.CIF_TRANS.getDescricao());
		}
		
		/*
		 * Devemos sempre pesquisar pois o cliente pode ter alterado os dados de logradouro
		 */
		...
		if (isPedidoNovo) {
			...
			// recuperando as informacoes do sistema que nao devem ser alteradas na edicao do pedido.
	...
	@Override
	public List<Pedido> pesquisarByIdCliente (Integer idCliente, Integer indiceRegistroInicial,  Integer numeroMaximoRegistros) {
	...
	public List<Pedido> pesquisarByIdClienteByIdVendedor (Integer idCliente, Integer idVendedor, 
		...
		return this.pedidoDAO.pesquisarByIdClienteByIdVendedor(idCliente, idVendedor, 
	...
	public Pedido removerItemPedido(Integer idItemPedido) throws BusinessException {
		...
		// Efetuando novamente o calculo pois na remocao o valor do pedido deve ser atualizado
		...
		return pedido;
			...
			throw new BusinessException("Não foi possivel remover o item pois foi encontrato mais de um item para o codigo "+idItemPedido);
	...
	public Double calcularValorPedido(Integer idPedido) throws BusinessException {
		final Double valor = QueryUtil.gerarRegistroUnico(this.entityManager.createQuery(
	...
	@Override
	public Integer inserirItemPedido(Integer idPedido, ItemPedido itemPedido, Double aliquotaIPI) throws BusinessException {
		...
		final Double precoUnidadeIPI = isCalculoIPIObrigatorio(itemPedido) 
				? CalculadoraPreco.calcularPorUnidadeIPI(itemPedido, aliquotaIPI) : itemPedido.getPrecoUnidade();
	...
	public List<Logradouro> pesquisarLogradouro(Integer idPedido) {
		return this.entityManager.createQuery("select l from Pedido p inner join p.listaLogradouro l where p.id = :idPedido")
	...
	private void enviarOrcamento(Pedido pedido, byte[] arquivoAnexado) throws BusinessException {
		...
		try {
			...
			mensagem.append("Falha no envio do orçamento No. ").append(pedido.getId())
				.append(" do vendedor ").append(pedido.getVendedor().getNomeCompleto())
				.append(" para o cliente ").append(pedido.getCliente().getNomeCompleto())
	...
	private void enviarVenda(Pedido pedido, byte[] arquivoAnexado) throws BusinessException {
		...
		final List<LogradouroCliente> listaLogradouro = this.clienteService.pesquisarLogradouro(pedido.getCliente().getId()); 
		this.logradouroService.verificarListaLogradouroObrigatorio(listaLogradouro);
		/*
		 * Devemos sempre usar a lista do cliente pois o cliente pode ter alterado os dados de logradouro
		 */
		...
		try {
			...
			StringBuilder mensagem = new StringBuilder();
			mensagem.append("Falha no envio do pedido No. ").append(pedido.getId())
				.append(" do vendedor ").append(pedido.getVendedor().getNomeCompleto())
				.append(" para a representada ").append(pedido.getRepresentada().getNomeFantasia());
	...
	private boolean isCalculoIPIObrigatorio(ItemPedido itemPedido) throws BusinessException {
		if (itemPedido.getPedido() == null || itemPedido.getPedido().getId() == null) {
			throw new BusinessException("Não é possível verificar a obrigatoriedade do IPI pois pedido ainda não existe no sistema");
		}
		...
		final Integer idRepresentada = QueryUtil.gerarRegistroUnico(
				this.entityManager.createQuery(select).setParameter("idPedido", itemPedido.getPedido().getId()), 
				Integer.class, null);
	...
	public Integer refazerPedido(Integer idPedido) throws BusinessException {
		...
		for (ItemPedido itemPedido : listaItemPedido) {
			try {
				...
				throw new BusinessException("Falha no processo de copia do item No. "+itemPedido.getId()+" do pedido No. "+idPedido, e);
			}
		}
		
		// Ao final da clonaem do pedido precisamos cancelar o antigo para que esse nao aparece nos faturamentos da empresa.
