...
public class UsuarioServiceImpl implements UsuarioService {
	...
	public PaginacaoWrapper<Usuario> paginarUsuario (Usuario filtro, boolean isVendedor, Boolean apenasAtivos,  
		...
		return new PaginacaoWrapper<Usuario>(
				this.pesquisarTotalRegistros(filtro, apenasAtivos, isVendedor), 
				this.pesquisar(filtro, isVendedor, apenasAtivos, indiceRegistroInicial, numeroMaximoRegistros));
		
	}
	
	@Override
	public PaginacaoWrapper<Usuario> paginarVendedor (Usuario filtro, Boolean apenasAtivos,  Integer indiceRegistroInicial, Integer numeroMaximoRegistros) {
		return new PaginacaoWrapper<Usuario>(
				this.pesquisarTotalRegistros(filtro, apenasAtivos, true), 
				this.pesquisar(filtro, true, apenasAtivos, indiceRegistroInicial, numeroMaximoRegistros));
	...
	public String pesquisarSenhaByEmail(String email) {
		return QueryUtil.gerarRegistroUnico(
				this.entityManager.createQuery("select u.senha from Usuario u where u.email = :email ")
				.setParameter("email", email), String.class, null);
	}
	
	@Override
	public Usuario pesquisarUsuarioResumidoById(Integer idUsuario) {
		return QueryUtil.gerarRegistroUnico(
				this.entityManager.createQuery("select new Usuario(u.id, u.nome, u.sobrenome) from Usuario u where u.id = :idUsuario ")
	...
	@Override
	public void associarCliente(Integer idVendedor, List<Integer> listaIdClienteAssociado, List<Integer> listaIdClienteDesassociado) throws BusinessException {
		if (idVendedor == null) {
	...
	public void desassociarCliente(Integer idVendedor, List<Integer> listaIdClienteDesassociado) throws BusinessException {
		...
		this.entityManager.createQuery("update Cliente c set c.vendedor = null where c.vendedor.id = :idVendedor and c.id in (:listaIdClienteDesassociado)")
			.setParameter("idVendedor", idVendedor)
			.setParameter("listaIdClienteDesassociado", listaIdClienteDesassociado)
			.executeUpdate();
	}
	...
	@Override
	public List<Usuario> pesquisarBy(Usuario filtro, Boolean apenasAtivos,  Integer indiceRegistroInicial, Integer numeroMaximoRegistros) {
	...
	public Usuario pesquisarById(Integer id) {
		return QueryUtil.gerarRegistroUnico(
				this.entityManager.createQuery("select c from Usuario c where c.id = :id")
	...
	public Usuario pesquisarVendedorById(Integer idVendedor) {
		...
		return QueryUtil.gerarRegistroUnico(
				this.entityManager.createQuery(
						"select c from Usuario c where c.id = :idVendedor and c.vendedorAtivo = true")
	...
	public List<ContatoUsuario> pesquisarContatos (Integer id) {
		Query query = this.entityManager.createQuery("select c from Usuario u inner join u.listaContato c where u.id =:id ");
		query.setParameter("id", id);
		return query.getResultList();
	}
	...
	public List<Remuneracao> pesquisarRemuneracaoById (Integer id) {
		Query query = this.entityManager.createQuery("select r from Usuario v inner join v.listaRemuneracao r where v.id = :id");
		query.setParameter("id", id);
		return query.getResultList();
	}

	@Override
	public List<Usuario> pesquisarVendedores(Usuario filtro, Boolean apenasAtivos,  Integer indiceRegistroInicial, Integer numeroMaximoRegistros) {
	...
	public boolean isClienteAssociadoVendedor(Integer idCliente, Integer idVendedor) {
		Query query = this.entityManager.createQuery("select c.id from Cliente c where c.id =:id and c.vendedor.id = :idVendedor ");
	...
	public boolean isVendedorAtivo(Integer idVendedor) {
		return QueryUtil.gerarRegistroUnico(
				this.entityManager.createQuery("select v.ativo from Usuario v where v.id =:idVendedor and v.vendedorAtivo = true")
