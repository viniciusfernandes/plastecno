...
<body>
	...
	<form id="formPesquisa" action="<c:url value="/material/listagem"/>" method="get">
	...
	<form id="formMaterial" action="<c:url value="/material/inclusao"/>"
		method="post">
		<input type="hidden" id="codigo" name="material.id" value="${material.id}"> 
		
		<fieldset>
			<legend>::: Dados do Material :::</legend>
			<div class="label" >Ativo:</div>
			<div class="input" style="width: 80%">
				<input type="checkbox" id="ativo" name="material.ativo" <c:if test="${empty material or material.ativo}">checked="checked"</c:if> class="checkbox"/>
			</div>
			<div class="label" >Importado:</div>
			<div class="input" style="width: 80%">
				<input type="checkbox" id="ativo" name="material.importado" <c:if test="${not empty material and material.importado}">checked="checked"</c:if> class="checkbox"/>
			</div>
			...
			<div class="input" style="width: 10%">
				<input type="text" id="pesoEspecifico" name="material.pesoEspecifico" value="${material.pesoEspecifico}" />
			</div>
		</fieldset>
		<div class="bloco_botoes">
			<a id="botaoPesquisarMaterial" title="Pesquisar Dados do Material" class="botaoPesquisar"></a>
			<a id="botaoLimpar" title="Limpar Dados do Material" onclick="limparFormulario();" class="botaoLimpar"></a>
		</div>
	...
	<div class="bloco_botoes">
		<c:if test="${acessoCadastroBasicoPermitido}">
			<a id="botaoInserirMaterial" title="Incluir Dados do Material" class="botaoInserir"></a>
		</c:if> 
	</div>
	</form>
								...
								<td>
									<div class="coluna_acoes_listagem">
										<form action="<c:url value="/material/edicao"/>" method="post">
											<input type="submit" title="Editar Dados do Material" value="" class="botaoEditar" />
											<input type="hidden" name="id" value="${material.id}" />
										</form>
										<c:if test="${acessoCadastroBasicoPermitido}">
											<form action="<c:url value="/material/desativacao"/>" method="post">
												<input type="hidden" name="id" value="${material.id}">
												<input type="submit" id="botaoRemoverRamo" title="Desativar Material" value="" class="botaoRemover"
