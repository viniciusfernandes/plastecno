	...
	<div id="content">
		...
		<form action="<c:url value="/login/sair"/>" method="get">
			<div class="bloco_autenticacao flutuante_esquerda">
				<input type="submit" value="" class="botaoLogout" title="Sair no sistema"/>
				<label class="bemVindo">Bem-vindo, <c:out value="${usuarioInfo.descricaoLogin}"/></label>
			</div>
		</form>
		...
		<nav class="flutuante_esquerda">
			<ul>
				<li><a href="<c:url value="/"/>">Início</a>
				</li>
				<li><a href="pedido" target="principal_frame">Pedidos</a>
				<li><a href="cliente" target="principal_frame">Clientes</a>
				</li>
				...
				<li><a href="javascript: void(0)">Relatórios</a>
					<ul>
						...
						<c:if test="${acessoRelatorioVendasRepresentadaPermitido}">
							<li><a href="relatorio/venda/periodo" target="principal_frame">Venda Período</a></li>
						</c:if>
				...
				<c:if test="${acessoManutencaoPermitido}">
					<li><a href="javascript: void(0)">Manutenção</a>
						<ul>
							<li><a href="administracao/log" target="principal_frame">Download Log Servidor</a></li>
							<li><a href="administracao/sql" target="principal_frame">SQL Editor</a></li>						
						</ul>
					</li>	
				</c:if>			
			</ul>
