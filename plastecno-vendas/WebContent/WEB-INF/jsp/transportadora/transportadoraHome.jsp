...
<body>
	...
	<form id="formVazio" action="transportadora" method="get">
	</form>
	...
	<form id="formTransportadora" action="<c:url value="/transportadora/inclusao"/>" method="post">
		<input type="hidden" id="codigo" name="transportadora.id" value="${transportadora.id}">
		...
		<legend>::: Dados de Transportadora :::</legend>
					...
					<div class="input" style="width: 80%">
						<input type="checkbox" id="ativo" name="transportadora.ativo" <c:if test="${empty transportadora or transportadora.ativo}">checked="checked"</c:if> class="checkbox"/>
					</div>		
					<div class="label obrigatorio" >Nome Fantasia:</div>
					<div class="input" style="width: 80%">
						<input type="text" id="nomeFantasia" name="transportadora.nomeFantasia" value="${transportadora.nomeFantasia}" class="pesquisavel" style="width: 60%"/>
					</div>
		
					<div class="label obrigatorio">Razão Social:</div>
					<div class="input" style="width: 80%">
						<input type="text" id="razaoSocial" name="transportadora.razaoSocial" value="${transportadora.razaoSocial}" class="pesquisavel" style="width: 60%"/>
					</div>
					
					<div class="label">CNPJ:</div>
					<div class="input" style="width: 20%">
						<input type="text" id="cnpj" name="transportadora.cnpj"  value="${transportadora.cnpj}" style="width: 80%" class="pesquisavel"/>
					</div>
		
					<div class="label" style="width: 11%">Insc. Estadual:</div>
					<div class="input" style="width: 40%">
						<input type="text" id="inscricaoEstadual" name="transportadora.inscricaoEstadual" value="${transportadora.inscricaoEstadual}" style="width: 40%; text-align: right;" />
					</div>
					
					<div class="label">Site:</div>
					<div class="input" style="width: 80%">
						<input type="text" id="site" name="transportadora.site" value="${transportadora.site}" style="width: 60%" 
					...
					<div class="input areatexto" style="width: 80%">
						<textarea id="areaAtuacao" name="transportadora.areaAtuacao" style="width: 60%">${transportadora.areaAtuacao}</textarea>
					</div>
		</fieldset>
		<div class="bloco_botoes" >
			<a id="botaoPesquisarTransportadora" title="Pesquisar Dados da Transportadora" class="botaoPesquisar"></a>
			<a id="botaoLimpar" title="Limpar Dados Da Transportadora" class="botaoLimpar"></a>
		</div>
	...
	<div class="bloco_botoes" >
		<c:if test="${acessoCadastroBasicoPermitido}">
			<a id="botaoInserirTransportadora" title="Incluir Dados da Transportadora" class="botaoInserir" ></a>
		</c:if>
	</div>
						...
						<tr>
							...
							<td>
								<div class="coluna_acoes_listagem">
									<form action="<c:url value="/transportadora/${transportadora.id}"/>" method="get">
										<input type="submit" title="Enviar Dados do(a) Transportadora" value="" class="botaoEditar" />
									</form>
									<c:if test="${acessoCadastroBasicoPermitido}">
										<form action="<c:url value="/transportadora/desativacao"/>" method="post">
											<input type="hidden" name="idTransportadora" value="${transportadora.id}">
											<input type="submit" title="Remover Dados do(a) Transportadora" value="" class="botaoRemover" 
												onclick="javascript: return confirm('Voce tem certeza de que deseja desativar a TRANSPORTADORA?');"/>
										</form>
									</c:if>
								</div>
							</td>
