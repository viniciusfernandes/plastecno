...
<legend>::: Endereço :::</legend>
		...
		<c:if test="${tipoLogradouroRenderizado}">
			...
			<div class="input" style="width: 80%">
				<select id="tipoLogradouro" name="logradouro.tipoLogradouro" style="width: 15%">
					<option value="">&lt&lt SELECIONE &gt&gt</option>
					<c:forEach var="tipo" items="${listaTipoLogradouro}">
						<option value="${tipo}" <c:if test="${tipo eq tipoLogradouroSelecionado}">selected</c:if>>${tipo}</option>
					</c:forEach>
				</select>
			</div>
		</c:if>
		...
		<div class="input" style="width: 10%">
			<input type="text" id="cep" name="logradouro.cep" maxlength="8" value="${logradouro.cep}" />
		</div>
		<div class="input" style="width: 5%">
			<input type="button" id="botaoPesquisaEndereco" title="Pesquisar Endereço" value="" class="botaoPesquisarPequeno" style="width: 20px"/>
		</div>
		<div class="label" style="width: 10%">End. codificado:</div>
		<div class="input" style="width: 50%">
			<input type="checkbox" id="codificado" name="logradouro.codificado" <c:if test="${empty logradouro or (not empty logradouro and logradouro.codificado)}">checked="checked"</c:if> class="checkbox"/>
		</div>
		<div class="label" >Endereço:</div>
		<div class="input" style="width: 40%">
			<input type="text" id="endereco" name="logradouro.endereco" value="${logradouro.endereco}" class="uppercaseBloqueado"/>
		</div>
		<div class="label" style="width: 8%">Número:</div>
		<div class="input" style="width: 30%">
			<input type="text" id="numero" name="logradouro.numero" value="${logradouro.numero}" maxlength="5" style="width: 25%"/>
		</div>
		<div class="label" >Complemento:</div>
		<div class="input" style="width: 70%">
			<input type="text" id="complemento" name="logradouro.complemento" value="${logradouro.complemento}" class="uppercaseBloqueado" style="width: 57%"/>
		</div>
		<div class="label" >Cidade:</div>
		<div class="input" style="width: 80%">
			<input type="text" id="cidade" name="logradouro.cidade" value="${logradouro.cidade}" style="width: 40%" class="uppercaseBloqueado"/>
		</div>
		<div class="label" >Bairro:</div>
		<div class="input" style="width: 80%">
			<input type="text" id="bairro" name="logradouro.bairro" value="${logradouro.bairro}" style="width: 40%" class="uppercaseBloqueado"/>
		</div>
		<div class="label" >UF:</div>
		<div class="input" style="width: 5%">
			<input type="text"  id="uf" name="logradouro.uf" value="${logradouro.uf}" />
		</div>
		<div class="label" style="width: 5%">País:</div>
		<div class="input" style="width: 21%">
			<input type="text" id="pais" name="logradouro.pais" value="${logradouro.pais}" class="uppercaseBloqueado"/>
		</div>
		...
		<c:if test="${possuiMultiplosLogradouros}">
			<div style="width: 100%; margin-top: 15px;">
				<table id="tabelaLogradouro" class="listrada">
					...
					<tbody>
						<c:forEach items="${listaLogradouro}" var="logradouro" varStatus="status">
